    ...
    function initFixProductBorder()
    {
    	if($('.product_filter').length)
    	{
			...
			else
			{
				...
				for(var i = 3; i < products.length; i+=5)
				{
					var product = $(products[i]);
					product.css('border-right', 'none');
				}
    	...
    	if($('.product-grid').length)
    	{
	        ...
	        sortNums.each(function()
	        {
	        	$(this).on('click', function()
	        	{
					...
					console.log(numSortingText);
	        		$('.num_sorting_text').text($(this).text());
    				$('.product-grid').isotope({filter: ':nth-child(-n+' + numSortingText + ')' });
	        	});
	        });	
    ...
    function initPriceSlider()
    {
		$( "#slider-range" ).slider(
		{
			...
			slide: function( event, ui )
			{
				...
				var minimal = formatRupiah(ui.values[ 0 ]);
				var maximal = formatRupiah(ui.values[ 1 ]);
				$( "#amount" ).val( "Rp " + minimal + " - Rp " + maximal );
			}
		});
			
		var minimal = formatRupiah($( "#slider-range" ).slider( "values", 0 ));
		var maximal = formatRupiah($( "#slider-range" ).slider( "values", 1 ));
		$( "#amount" ).val( "Rp " + minimal + " - Rp " + maximal );
    }
