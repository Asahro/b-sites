...
<?php $this->load->view('web/themplate/headbar'); ?>
	...
	<?php 
	    ...
	    if(!strpos($curenturl, '?')){
	        $curenturl = $curenturl."?page=1&sortir=nama-asc";
	    }
	?>
...
<!-- 
						<?php 
  				    	    ...
  				    	    for($p = 0; $p < count($listtype); $p++ ) { 
								...
								$cut = explode("?", $curenturl);
								$cut = explode("daftar", $cut[0]);
								$sorturl = str_replace("semua-", "", $curenturl);
								$sorturl = str_replace("daftar".$cut[1], $listtype[$p]['link_list'], $sorturl);
								$sorturl = str_replace("page=".$page, "page=1", $sorturl);

  				    	    	?>

  				    	    	<li 
  				    	    	<?php if($listtype[$p]['link_list'] == "daftar".$cut[1]){ ?>
  				    	    		class="active"
	  				    	    <?php } ?>
	  				    	    >
 ...
 -->
								...
								<div class="product_sorting_container product_sorting_container_top row">
									...
									<ul class="product_sorting col-4">
										...
										<li class="col-md-12">
											...
											<?php 
												$sorturl = str_replace("&sortir=".$sortir, "", $curenturl);
												$sorturl = str_replace("page=".$page, "page=1", $sorturl);
												$sortir = explode("-", $sortir);
												echo ucfirst($sortir[0])." ".strtoupper($sortir[1]) ?>	
											</span>
	...
	<script src="<?php echo base_url() ?>nassets/js/categories_custom.js"></script>